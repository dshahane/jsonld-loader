...
use log::warn;
// Added PgRow and Row for manual mapping in sparql_select

/// Resolution cache with TTL support
...
/// Main schema ingester with all features
pub struct JsonLdSchemaIngester {
    client: PgPool, // Changed from Client to PgPool
    cache: Option<Arc<ResolutionCache>>,
    enable_cache: bool,
    context_cache: Option<Arc<Value>>,
}

// Added sqlx::FromRow to enable automatic mapping
...
impl JsonLdSchemaIngester {
    ...
    pub async fn drop_schema_table(&self, table_name: &str) -> Result<()> {
    ...
    /// Parse JSON-LD file
    pub fn parse_jsonld_file(&self, file_path: &str) -> Result<Vec<Value>> {
    ...
    async fn read_and_parse_file(&self, file_path: &str) -> Result<Value> {
        use tokio::fs;
    ...
    pub async fn get_jsonld_context(&self, _file_path: &str) -> Result<Value> {
    ...
    pub async fn bulk_resolve(
        ...
        table_name: &str,
        max_depth: usize,
    ) -> Result<HashMap<String, ClassData>> {
        if uris.is_empty() {
        ...
        // ✅ Corrected: Use query_as and fetch_all to map directly to Vec<ClassData>
    ...
    pub async fn sparql_select(
        ...
        let results: Vec<(String, Value)> = rows
            .into_iter()
            .map(|row| {
                // ✅ Use row.try_get::<T>(index/name) for manual retrieval from PgRow
